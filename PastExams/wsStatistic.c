...
int main()
{
	...
	{	
		...
		{
			if(!strncmp(filename, "/reflect",8))
			{
				sprintf(rq, "%s %s %s\r\n", method, filename, version);
				f=fopen("reflect.txt", "w");
				fwrite(rq, sizeof(char),strlen(rq), f);
				for(i=0; i<j;i++)
				{
					fwrite(h[i].n,sizeof(char), strlen(h[i].n),f);
					fwrite(":", sizeof(char),1, f);
					fwrite(h[i].v, sizeof(char), strlen(h[i].v),f);
					fwrite("\n", sizeof(char), 1,f);
				}				
				fwrite("\r\n",sizeof(char), 1,f);
				char ip[20];
				unsigned char* pointer=(unsigned char*)&remote.sin_addr.s_addr;
				sprintf(ip,"client ip: %d.%d.%d.%d\r\nclient port: %d\r\n",pointer[0],pointer[1], pointer[2], pointer[3],remote.sin_port);
				fwrite(ip, sizeof(char),strlen(ip),f);
				fclose(f);
				printf("%s\n", ip);
				sprintf(filename,"/reflect.txt");

...
int val=1;
if(-1==setsockopt(s2,SOL_SOCKET, SO_KEEPALIVE,&val, sizeof(int))){perror("Socket opt fallita:"); exit(1);}
int z=0;
...
while(z<3)
{
	...
	for(i=0;i<j;i++)
		if(!strcmp("Connection",h[i].n))
			if(!strcmp("Close",h[i].v))
				z=3;
	...
	if ( !strcmp(method,"GET")){
		...
		for(i=0; i<j; i++)
		{
			if(!strcmp(h[i].n, "Transfer-Encoding"))//Transfer-Encoding
				c=1;
